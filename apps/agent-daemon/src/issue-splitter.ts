...
const REQUIRED_CHILD_CONTRACT_SECTIONS = [
  '## 用户故事',
  '## Context',
  '### Dependencies',
  '### Constraints',
...
export async function splitTrackingIssue(
  ...
  for (let index = 0; index < allocatedChildren.length; index += 1) {
    ...
    const splitValidationErrors = validateSplitChildContract(finalizedMarkdown)

    if (splitValidationErrors.length > 0) {
      throw new Error(`split child ${child.title} failed contract validation: ${splitValidationErrors.join(' | ')}`)
    }
...
function validateSplitChildContract(markdown: string): string[] {
  const errors: string[] = []
  const dependencyBlockPattern = /###\s+Dependencies\s*\n```json\s*\n[\s\S]*?\n```/gm
  const dependencyBlocks = markdown.match(dependencyBlockPattern) ?? []

  if (dependencyBlocks.length === 0) {
    errors.push('split child is missing a ### Dependencies fenced JSON block')
  }

  const outsideDependencyBlock = markdown.replace(dependencyBlockPattern, '')
  if (/\bdependsOn\b/.test(outsideDependencyBlock)) {
    errors.push('split child mentions dependsOn outside the ### Dependencies fenced JSON block')
  }

  for (const heading of REQUIRED_CHILD_CONTRACT_SECTIONS) {
    if (!markdown.includes(heading)) {
      errors.push(`split child is missing required section ${heading}`)
    }
  }
...
function renderDeterministicChildIssue(input: {
  ...
  const reviewHints = uniqueStrings([
    ...
    '优先检查 child issue 的执行顺序是否与拆分计划一致',
  ...
  return [
    ...
    renderNumberedSection('## 实现步骤', [
      ...
      '最后运行 Validation，并确认 OutOfScope 和 contract 结构没有回归',
    ]),
    '',
